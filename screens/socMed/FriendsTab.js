...
import FollowingTabScrn from '../socMed/Following'
import FollowersTabScrn from '../socMed/Followers'

//export default function ChatRoomPage({navigation}) {
  // Calling Plantcare search screen

  function funcFollowingTabScrn({navigation}) {
    return (
      <FollowingTabScrn navigation={navigation}/>
    );
  }
  function funcFollowersTabScrn({navigation}) {
    return (
      <FollowersTabScrn navigation={navigation}/>
    );
  }
...
function MyTopTabs({navigation}) {
  return (
    <Tab.Navigator
      initialRouteName="FriendsTabPage"
      screenOptions={{
    ...
    >
      <Tab.Screen
        name="FollowerTab"
        component={funcFollowersTabScrn}
        options={{ tabBarLabel: 'Followers' }}
      />
      <Tab.Screen
        name="FollowingTab"
        component={funcFollowingTabScrn}
